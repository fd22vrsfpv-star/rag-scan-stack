"""
Log Manager for Brutus Runner Diagnostic Web Interface
Captures logs from credential testing operations
"""
...
def setup_log_capture():
    for name in ["root", "brutus_runner", "brutus-runner"]:
        logger = logging.getLogger(name)
        if _log_handler not in logger.handlers:
            logger.addHandler(_log_handler)
            logger.setLevel(logging.INFO)
    logging.getLogger().addHandler(_log_handler)
    print(f"[brutus-runner] Log capture initialized: max {_log_handler.max_size} records", file=sys.stderr)
...
<head>
    ...
    <title>Brutus Runner Logs</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body { font-family: 'Segoe UI', sans-serif; background: #1a1a1a; color: #e0e0e0; padding: 20px; }
        .container { max-width: 1400px; margin: 0 auto; }
        header { background: linear-gradient(135deg, #c0392b 0%, #2c3e50 100%); padding: 30px; border-radius: 10px; margin-bottom: 30px; }
        ...
        button { background: #c0392b; color: white; border: none; padding: 10px 20px; border-radius: 5px; cursor: pointer; }
        ...
        .stat-value { font-size: 24px; font-weight: bold; color: #c0392b; }
        ...
        .log-entry.INFO { border-left-color: #c0392b; }
        ...
        .log-level.INFO { background: #c0392b; color: white; }
...
<body>
    <div class="container">
        <header><h1>Brutus Runner Logs</h1><p class="subtitle">Real-time logging for credential testing operations</p></header>
    ...
    <script>
        ...
        async function exportLogs() { const r = await fetch('/logs/export'); const b = await r.blob(); const a = document.createElement('a'); a.href = URL.createObjectURL(b); a.download = `brutus_runner_logs_${new Date().toISOString().replace(/[:.]/g,'-')}.json`; a.click(); }
