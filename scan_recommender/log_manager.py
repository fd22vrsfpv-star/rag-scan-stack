"""
Log Manager for Scan Recommender Diagnostic Web Interface
Captures logs from exploits_rag and scan_recommender and provides query interface

Based on autogen_agents/log_manager.py
"""
...
def setup_log_capture():
    """
    Setup log capture for scan-recommender loggers
...
<head>
    ...
    <title>ExploitDB RAG Logs</title>
    <style>
...
<body>
    <div class="container">
        <header>
            <h1>ExploitDB RAG Logs</h1>
            <p class="subtitle">Real-time logging for ExploitDB ingestion and RAG operations</p>
        </header>
        ...
        <div class="controls">
            ...
            <div class="control-group auto-refresh">
                ...
                <span style="font-size: 12px; color: #888;">Every 5s</span>
            </div>
        </div>
    ...
    <script>
        ...
        async function exportLogs() {
            try {
                ...
                a.href = url;
                a.download = `exploitdb_logs_${new Date().toISOString().replace(/[:.]/g, '-')}.json`;
                a.click();
            } catch (error) {
