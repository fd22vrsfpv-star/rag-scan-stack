...
def get_tool_coverage():
    ...
    * `uncatalogued` — a tool that IS installed (often provisioned onto a node)
    ...
    try:
        kb = get_tool_kb()
        for svc, spec in (kb._data.get("services") or {}).items():
            for t in (spec.get("tools") or []):
                name = (t.get("name") or "").strip().lower()
                if name:
                    referenced.setdefault(name, set()).add(svc)
    except Exception as e:
        logger.warning("tool-coverage: KB unreadable (%s)", e)

    try:
        with get_db() as conn, conn.cursor(cursor_factory=RealDictCursor) as cur:
            cur.execute("SELECT service_name, data FROM public.kb_service_overrides")
            for row in cur.fetchall():
                for t in ((row["data"] or {}).get("tools") or []):
                    name = (t.get("name") if isinstance(t, dict) else str(t) or "").strip().lower()
                    if name:
                        referenced.setdefault(name, set()).add(f"override:{row['service_name']}")
    except Exception as e:
        logger.debug("tool-coverage: overrides unavailable (%s)", e)
    ...
    # Installed-but-unknown. Drawn from CURATED sources, not the raw binary
    # sweep: keyword-matching 1,800 binaries surfaced apt-ftparchive,
    # dpkg-preconfigure and fc-scan as "security tools", which buries the few
    # that matter. A node's `capabilities` is what provisioning deliberately
    # installed, so every entry there is a real candidate by construction.
    known = set(referenced)
    curated: set = set()
    try:
        with get_db() as conn, conn.cursor(cursor_factory=RealDictCursor) as cur:
            cur.execute("SELECT capabilities FROM public.remote_nodes "
                        "WHERE capabilities IS NOT NULL")
            for row in cur.fetchall():
                for t in (row["capabilities"] or []):
                    if isinstance(t, str) and t.strip():
                        curated.add(t.strip().lower())
    except Exception as e:
        logger.debug("tool-coverage: node capabilities unavailable (%s)", e)

    interesting = {t for t in curated if t not in known and t in binaries}
    return {
        "ok": True,
...
@kb_router.post("/tools/adopt")
def adopt_tool(body: AdoptToolBody):
    """Add an installed-but-unused tool to a service's KB override.

    Closes the loop the coverage check opens: provisioning installs a tool, the
    KB never mentions it, so it can never be recommended and the capability is
    paid for but unusable. This writes it into kb_service_overrides, which
    generate_recommendations consults BEFORE the YAML.

    Refuses tools that are not actually installed — adopting one would create
    exactly the `unrunnable` entries the coverage check exists to surface.
    """
    from tool_catalog import load_catalogs, _SCANNER_BINARY

    tool = (body.tool or "").strip().lower()
    service = (body.service or "").strip().lower()
    if not tool or not service:
        raise HTTPException(400, "tool and service are both required")
    if not re.fullmatch(r"[a-z0-9][a-z0-9._+-]{0,63}", tool):
        raise HTTPException(400, f"Invalid tool name: {body.tool!r}")
    if not re.fullmatch(r"[a-z0-9][a-z0-9._+-]{0,63}", service):
        raise HTTPException(400, f"Invalid service name: {body.service!r}")

    binaries = load_catalogs().get("binaries") or set()
    if binaries:
        mapped = _SCANNER_BINARY.get(tool, tool)
        if mapped is not None and mapped not in binaries:
            raise HTTPException(
                400,
                f"{tool!r} is not installed anywhere the stack can reach — adopting it "
                f"would create a recommendation that fails at dispatch. Provision it "
                f"first, or add it to knowledge/tool_catalogs.local.json if it lives "
                f"somewhere the probe cannot see.")

    entry = {"name": tool, "purpose": (body.purpose or f"{tool} against {service}").strip()}
    if body.command:
        entry["command"] = body.command.strip()

    try:
        with get_db() as conn, conn.cursor(cursor_factory=RealDictCursor) as cur:
            cur.execute("SELECT data FROM public.kb_service_overrides WHERE service_name = %s",
                        (service,))
            row = cur.fetchone()
            data = dict(row["data"]) if row else {}
            tools = list(data.get("tools") or [])
            # Seed from the YAML on first override so adopting one tool does not
            # silently REPLACE the service's existing recommendations — the
            # override is consulted before the YAML and wins outright.
            if not row:
                try:
                    base = get_tool_kb().get_tools_for_service(service=service) or {}
                    tools = list(base.get("tools") or [])
                except Exception as e:
                    logger.debug("adopt: no YAML base for %s (%s)", service, e)
            if any((t.get("name") if isinstance(t, dict) else t) == tool for t in tools):
                return {"ok": True, "unchanged": True,
                        "detail": f"{tool} already listed for {service}"}
            tools.append(entry)
            data["tools"] = tools
            cur.execute(
                "INSERT INTO public.kb_service_overrides (service_name, data) VALUES (%s, %s) "
                "ON CONFLICT (service_name) DO UPDATE SET data = EXCLUDED.data, "
                "    updated_at = now()",
        ...
        logger.error("adopt_tool failed for %s/%s: %s", tool, service, e)
        raise HTTPException(500, f"Could not adopt tool: {e}")

    _emit_webhook("kb_tool_adopted", {"tool": tool, "service": service,
                                      "total_tools": len(tools)})
    return {"ok": True, "service": service, "tool": tool, "total_tools": len(tools)}
