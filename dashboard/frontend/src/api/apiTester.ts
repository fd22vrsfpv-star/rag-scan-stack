...
export function useSendToPipeline() {
  return useMutation({
    mutationFn: (data: { collection_id: string; target_url?: string }) =>
      apiFetch<{ ok: boolean; urls_sent: number }>('/api-test/send-to-pipeline', {
        method: 'POST',
        body: JSON.stringify(data),
      }),
  })
...
export function useCollectionToScope() {
  const qc = useQueryClient()
  return useMutation({
    mutationFn: (data: { collection_id: string; scope_name: string }) =>
      apiFetch<{ ok: boolean; added: number; total: number; scope_name: string }>(
        `/api-collections/${data.collection_id}/to-scope`,
        { method: 'POST', body: JSON.stringify({ scope_name: data.scope_name }) },
      ),
    onSuccess: () => {
...
export function useCaptureAuth() {
  return useMutation({
    mutationFn: (data: {
      ...
      apiFetch<{ ok: boolean; access_token?: string; error?: string; mode?: string }>('/auth/capture', {
        method: 'POST',
        body: JSON.stringify(data),
      }),
  })
