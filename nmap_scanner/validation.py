...
def validate_scan_target(target: str, allow_private: bool = False) -> str:
    ...
    try:
        ...
        pass  # not a valid IP — fall through to CIDR / domain checks

    # Try to parse as CIDR network range (e.g. 192.168.1.0/24)
    if '/' in target:
        try:
            ...
            if not allow_private:
                if network.is_private or network.is_loopback:
                    raise ValidationError(
                        f"Cannot scan private/internal networks: {target}"
                    )

            return str(network)

        except ValueError:
            raise ValidationError(f"Invalid CIDR notation: {target}")

    # Not an IP or CIDR, validate as domain name
