"""Named port scope profiles for the AutoGen agents.

VERBATIM COPY of dashboard/bff/services/port_profiles.py — the two containers
cannot share Python, but they DO share the underlying data
(knowledge/port_profiles.yaml, bind-mounted read-only into both), so the
profiles themselves can never drift.  Keep this file in sync when editing the
BFF copy; the shared behaviour is covered by tests/test_port_profiles.py.
