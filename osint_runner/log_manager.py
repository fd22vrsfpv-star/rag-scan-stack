"""
Log Manager for OSINT Runner Diagnostic Web Interface
Captures logs from passive reconnaissance tool operations
"""
...
def setup_log_capture():
    for name in ["root", "osint_runner", "osint-runner"]:
        logger = logging.getLogger(name)
        if _log_handler not in logger.handlers:
            logger.addHandler(_log_handler)
            logger.setLevel(logging.INFO)
    logging.getLogger().addHandler(_log_handler)
    print(f"[osint-runner] Log capture initialized: max {_log_handler.max_size} records", file=sys.stderr)
...
<head>
    ...
    <title>OSINT Runner Logs</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body { font-family: 'Segoe UI', sans-serif; background: #1a1a1a; color: #e0e0e0; padding: 20px; }
        .container { max-width: 1400px; margin: 0 auto; }
        header { background: linear-gradient(135deg, #27ae60 0%, #2c3e50 100%); padding: 30px; border-radius: 10px; margin-bottom: 30px; }
        ...
        .log-level.INFO { background: #27ae60; color: white; }
...
<body>
    <div class="container">
        <header><h1>OSINT Runner Logs</h1><p class="subtitle">Real-time logging for passive reconnaissance operations</p></header>
    ...
    <script>
        ...
        async function exportLogs() { const r = await fetch('/logs/export'); const b = await r.blob(); const a = document.createElement('a'); a.href = URL.createObjectURL(b); a.download = `osint_runner_logs_${new Date().toISOString().replace(/[:.]/g,'-')}.json`; a.click(); }
