"""
Log Manager for Web Scanner Diagnostic Web Interface
Captures logs from ZAP and web scanning operations
"""
...
def setup_log_capture():
    for name in ["root", "web_scanner", "zap"]:
        logger = logging.getLogger(name)
        if _log_handler not in logger.handlers:
            logger.addHandler(_log_handler)
            logger.setLevel(logging.INFO)
    logging.getLogger().addHandler(_log_handler)
    print(f"[web-scanner] Log capture initialized: max {_log_handler.max_size} records", file=sys.stderr)
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .stat-value { font-size: 24px; font-weight: bold; color: #9b59b6; }
        ...
        .log-entry.INFO { border-left-color: #9b59b6; }
        ...
        .log-level.INFO { background: #9b59b6; color: white; }
...
<body>
    <div class="container">
        <header><h1>Web Scanner Logs</h1><p class="subtitle">Real-time logging for ZAP web scanning operations</p></header>
    ...
    <script>
        ...
        async function exportLogs() { const r = await fetch('/logs/export'); const b = await r.blob(); const a = document.createElement('a'); a.href = URL.createObjectURL(b); a.download = `web_scanner_logs_${new Date().toISOString().replace(/[:.]/g,'-')}.json`; a.click(); }
